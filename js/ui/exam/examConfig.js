...
function examConfigVocabPanelHtml(goal,activeSkill){
  ...
  const sub=isGenModule
    ?(isDE
      ?'Die KI kann diese Wörter einbauen, wenn sie natürlich passen — keine Pflicht, alle zu verwenden.'
      :'The AI may include these words if they fit naturally — not required to use all of them.')
    :(isDE
  ...
  return`<div class="ec-vocab-card">
    <div class="ec-vocab-head"><span class="ec-vocab-title">${title}</span><span class="ec-vocab-count">${selN} ${isDE?'ausgewählt':'selected'}</span></div>
...
function renderExamConfigurator(){
  ...
  const topicHtml=activeSkill!=='lesen'?'':configTopicSelectHtml(goal);
