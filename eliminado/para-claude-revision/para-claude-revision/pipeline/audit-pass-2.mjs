...
import { BLACKLIST } from './lib/blacklist.mjs';
...
function chk4(batch, file) {
  ...
  for (const [key, qs] of Object.entries(groups)) {
    ...
    if (type === 'multiple_choice' || (type === 'matching' && key.startsWith('horen-4'))) {
      ...
      // No single letter > 45% (or for horen-4 a>2/8=25% ok, ≤50%)
      for (const [v, cnt] of Object.entries(dist)) {
        const pct = cnt / n;
        const limit = key.startsWith('horen-4') && v === 'a' ? 0.50 : 0.50;
        if (pct > limit) {
          findings.push(finding('CHK-4', 'IMPORTANT', file, key,
            `Balance MC: "${v}"=${Math.round(pct*100)}% supera el máximo 50%. Dist: ${JSON.stringify(dist)}`));
        }
      }
      ...
      if (n >= 5) {
        for (const [v, cnt] of Object.entries(dist)) {
          ...
          if (pct < 0.35 || pct > 0.65) {
            findings.push(finding('CHK-4', 'IMPORTANT', file, key,
              `Balance R/F o Ja/Nein: "${v}"=${Math.round(pct*100)}% fuera de rango 35–65%. Dist: ${JSON.stringify(dist)}`));
          }
        }
      }
    }
...
// Palabras absolutas en enunciados Richtig/Falsch hacen los ítems demasiado transparentes.
...
function chk10(batch, file) {
  ...
  for (const q of batch.questions || []) {
    if (q.module !== 'lesen' || q.teil !== 1) continue;
    if (q.type !== 'richtig_falsch' && q.type !== 'true_false') continue;
    const stem = q.question || '';
    if (ABSOLUTE_WORDS_RE.test(stem)) infractors.push(q);
    if (ABSOLUTE_PHRASE_RE.test(stem)) {
      findings.push(finding('CHK-10', 'IMPORTANT', file, q.id,
        `Lesen T1: enunciado contiene la frase excesivamente obvia "ausschließlich täglich" — reescribir como contradicción de contenido.`));
    }
  }

  if (infractors.length > 1) {
    findings.push(finding('CHK-10', 'IMPORTANT', file, infractors[0].id,
      `Lesen T1: ${infractors.length} ítems con lenguaje absoluto (immer/nie/alle/ausschließlich…) — más de 1 reduce validez pedagógica.`));
  } else if (infractors.length === 1) {
    findings.push(finding('CHK-10', 'MINOR', file, infractors[0].id,
      `Lesen T1: ítem con palabra absoluta («${(infractors[0].question||'').match(ABSOLUTE_WORDS_RE)?.[0]}») — considerar reescribir como inferencia de contenido.`));
  }
  return findings;
}
...
function chk9(batch, file) {
  ...
  for (const q of batch.questions || []) {
    ...
    if (!hasBsp) {
      groups.add(key);
      findings.push(finding('CHK-9', 'MINOR', file, key,
...
function loadBatchFile(filePath) {
  ...
  // Pool format: {meta, passages, questions, vocabulary}
  // Batch format: {passages, questions}
  return {
...
function printFindings(findings, summaryOnly) {
  ...
  const counts = { CRITICAL: 0, IMPORTANT: 0, MINOR: 0, INFO: 0 };
