 ...
 * @returns {string|null} 16-char hex sha256 prefix, or null if not exactly 7 situations
 */
export function t3SituationFingerprintFromQuestions(questions) {
  const situations = (questions || [])
    .map((q) => normalizeT3SituationText(q?.question))
    .filter(Boolean);
  if (situations.length !== 7) return null;
