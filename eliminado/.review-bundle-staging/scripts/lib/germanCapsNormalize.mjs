...
/** Stable implementation tag — bump only after holdout-validated changes. */
export const GERMAN_CAPS_NORMALIZE_VERSION = 'v3.1-stable';
import {
  decapitalizeBatchMidSentence,
  capitalizeBatchNouns,
} from './capitalizeNouns.mjs';
import { normalizeBatchMcqOptionCapitalization } from './normalizeMcq.mjs';
...
export function applyGermanCapsNormalize(batch, opts = {}) {
  if (!batch || typeof batch !== 'object') {
    return { batch, stats: { decapFixed: 0, capFixed: 0, fieldsChanged: 0 }, changes: [] };
  }
  ...
  const changes = [];
  walkBatchStrings(normalized, (path, after) => {
  ...
  if (opts.log && (stats.decapFixed || stats.capFixed)) {
    console.log(
      `  [germanCapsNormalize] decap=${stats.decapFixed} cap=${stats.capFixed} fields=${stats.fieldsChanged}`,
    );
  }

  return { batch: normalized, stats, changes };
}
