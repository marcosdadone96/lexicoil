...
async function enrichFlashcardTranslations(fc){
  ...
  if(typeof fetchVocabCache==='function'){
    try{
      const hit=await fetchVocabCache(subject,lang,word);
      if(hit?.translation){
...
async function submitManualVocab(context){
  ...
  if(!result.ok){
    if(result.reason==='spelling'&&result.suggestion){
      ...
      showManualAddHint(context,'¿Quisiste decir <button type="button" class="vv-add-fix" onclick="useManualSuggestion(\''+context+'\')"><b>'+esc(result.suggestion)+'</b></button>?');
      lcToast('Revisa la ortografía — pulsa la palabra sugerida para usarla.','warn');
      return;
    }
...
// ═══════════════════════════════════════════
function veHintTypeLabel(hintType){
...
async function startVE(){
  ...
  try{
    ...
    questions=await generateVocabQuizWithAI(words,{lang:subject,level,hintLang,count:qCount});
  }catch(e){
    hideAll();
    veBackFromQuiz();
...
function renderVEQ(){
  ...
  const hintLbl=veHintTypeLabel(q.hintType);
  const veGoal=getActiveGoal();
  const subject=veGoal?.subject||S.deckGoalFilter||S.subject||'de';
